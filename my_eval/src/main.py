...
def main(gs_path, pred_path, subtask=['class', 'ner', 'norm'], codes_path='', g=None, p=None, doc_list=None):
    '''
    ...
    if subtask=='norm':
        ...
        gs = gs[['clinical_case', 'span', 'offset', 'label']].copy()
        gs = gs.rename(columns={'clinical_case': 'filename', 'offset': 'span', 'span': 'text', 'label': 'code'})
        pred = pred[['clinical_case', 'span', 'offset', 'label']].copy()
        pred = pred.rename(columns={'clinical_case': 'filename', 'offset': 'span', 'span': 'text', 'label': 'code'})
        ...
        pred['code'] = pred['code'].apply(lambda x: str(x).strip())
        pred['clinical_case'] = pred['clinical_case'].apply(lambda x: x.strip())
        
    elif subtask in ['class', 'ner', 'class_ner']:
        ...
        if 'code' in gs.columns:
            gs.drop(['code'], axis=1, inplace=True)
        if 'code' in pred.columns:
            pred.drop(['code'], axis=1, inplace=True)
    ...
    if subtask in ['ner', 'class', 'class_ner']:
        doc_list_gs = list(filter(lambda x: x[-4:] == '.ann', os.listdir(gs_path))) if doc_list is None else doc_list
    elif subtask == 'norm':
    ...
    if subtask=='norm':
        valid_codes = pd.read_csv(codes_path, sep='\t', header=0)['code'].tolist() if g is None else gs['code'].tolist()
